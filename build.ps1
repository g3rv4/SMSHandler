...
$basePath = Pwd
$csProjPath = Join-Path $basePath SMSHandler.csproj
...
docker run --rm -v "$($basePath):/var/src" mcr.microsoft.com/dotnet/core/sdk:3.1.404-alpine3.12 ash -c "dotnet publish -c Release /var/src/SMSHandler.csproj -o /var/src/bin/build && chown -R $($uid):$($gid) /var/src"

$nuspecPath = Join-Path $buildPath smshandler.nuspec
$nupkgPath = Join-Path $buildPath "smshandler.$($newVersion).nupkg"
cp smshandler.nuspec $nuspecPath
